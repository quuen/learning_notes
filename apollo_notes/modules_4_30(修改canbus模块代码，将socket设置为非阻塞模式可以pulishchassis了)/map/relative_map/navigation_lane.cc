...
bool NavigationLane::GeneratePath() {
  ...
  int navigation_line_num = navigation_info_.navigation_path_size();  ///< 参考线的数量
  const auto &lane_marker = perception_obstacles_.lane_marker(); ///< 车道标记信息
  ...
  if (config_.lane_source() == NavigationLaneConfig::OFFLINE_GENERATED &&
    ...
    ///< 由配置文件可知使用的是OFFLINE_GENERATED模式
    ...
    for (int i = 0; i < navigation_line_num; ++i) {
      ...
      if (ConvertNavigationLineToPath(i, path)) {
        ...
        navigation_path_list_.emplace_back(
            i, default_left_width_, default_right_width_, current_navi_path);///< 默认宽度在这也有用到
      }
    }
    ...
    double min_d = std::numeric_limits<double>::max(); /// 选出y坐标最大的值
    ...
    MergeNavigationLineAndLaneMarker(std::get<0>(current_navi_path_tuple_),
                                     path);
    ///< 最终使用的车道左右宽度使用的是感知部分做获取的值（navigation_line部分没有生成这部分信息）
    // Set the width for the navigation path which the vehicle is currently on.
    double left_width = perceived_left_width_ > 0.0 ? perceived_left_width_
                                                    : default_left_width_; /// 1.75m
    double right_width = perceived_right_width_ > 0.0 ? perceived_right_width_
                                                      : default_right_width_; /// 1.75m
...
bool NavigationLane::ConvertNavigationLineToPath(const int line_index,
  ...
  auto proj_index_pair = UpdateProjectionIndex(navigation_path, line_index);///< 更新车辆在第几个点
  // Can't find a proper projection index（感觉指的是第几个point） in the "line_index"（第几个参考线）lane according to
  // current vehicle position.
  int current_project_index = proj_index_pair.first; /// 当前导航线中车辆的投影点索引值。
...
///< navigation_line转换数据格式，使用的是车体坐标系。
///< 1.车辆当前位置所在的第几个path_point,2.总共多少个path_point,3.生成的最大长度，250, 
///< 4.Path类的对象指针
  auto gen_navi_path_loop_func =
      [this, &navigation_path, &enu_to_flu_func](
        ...
        CHECK_NOTNULL(path);
        const double ref_s = navigation_path.path_point(start).s(); /// 车辆在navigation_line投影点的s为基准
        for (int i = start; i < end; ++i) {
          ...
          const double accumulated_s =
              navigation_path.path_point(i).s() - ref_s + ref_s_base;///< navigation_line中每个点对应的s之间的差则为距离
  ...
  double dist = navigation_path.path_point().rbegin()->s() -
                navigation_path.path_point(current_project_index).s();///< 车辆当前投影索引点到终点的距离
  // Stitch current position to the beginning for a cyclic/circular route.
  if (FLAGS_enable_cyclic_rerouting && ///false
      dist < FLAGS_max_len_from_navigation_line) {///< 250.0
  ...
  if (dist < 20) { ///< 如果车辆即将到达navigation_line的终点也不不利用navigation_line进行转化，只用感知部分进行navigation_pah的生成
    return false;
  }

  gen_navi_path_loop_func(std::max(0, current_project_index - 3),///< 为什么减3？？？
                          navigation_path.path_point_size(), 0.0,
                          FLAGS_max_len_from_navigation_line, path);///< 规划距离最大为250m
    return true;
}
...
ProjIndexPair NavigationLane::UpdateProjectionIndex(const common::Path &path,
  ...
  auto item_iter = last_project_index_map_.find(line_index);///< 寻找键key=line_index的这条参考线
  if (item_iter != last_project_index_map_.end()) {
    current_project_index = std::max(0, item_iter->second.first);///< 当前车辆在导航线上的投影点索引
  }
  ...
  auto check_distance_func = [this, &path,
    ...
    if (std::fabs(flu_x) < FLAGS_max_distance_to_navigation_line / 2.0 &&
        std::fabs(flu_y) < FLAGS_max_distance_to_navigation_line * 2.0) {///< 检查投影点距离车辆横向距离小于30，纵向距离小于7.5 
      return true;
    }
    return false;
  };
  ...
  for (int i = current_project_index; i + 1 < path_size; ++i) {
    ...
    if (current_project_index != 0 && d > kMaxDistance) {///< 如果投影点到车辆距离大于50,也不用继续循环了，距离最小的投影点肯定在前边的循环范围内
      break;
    }
  }
  ...
  if (check_distance_func(index)) {
    if (FLAGS_enable_cyclic_rerouting) {
      ...
      if (item_iter != stitch_index_map_.end()) {
        int start_index =
            std::max(item_iter->second.first, item_iter->second.second); ///类比前边函数 m，n
        int end_index =
            std::min(item_iter->second.first, item_iter->second.second);
    ...
    return std::make_pair(index, min_d);///< 确定当前投影点索引和距离车辆距离
  }
...
void NavigationLane::ConvertLaneMarkerToPath(
  ...
  double path_c0 = (left_lane.c0_position() + right_lane.c0_position()) / 2.0;///< 求中心位置
  ...
  double path_range = current_speed * FLAGS_ratio_navigation_lane_len_to_speed; ///< 8.0s
  if (path_range <= FLAGS_min_len_for_navigation_lane) {///< 150.0
    path_range = FLAGS_min_len_for_navigation_lane;
  } else {
    path_range = FLAGS_max_len_for_navigation_lane; ///< 250.0
  }
  ...
  for (double z = start_s; z <= path_range; z += unit_z) {
    double x1 = z;
    double y1 = 0;
    if (left_lane.view_range() > FLAGS_min_view_range_to_use_lane_marker ||///< 0.5
    ...
    if (path->path_point_size() > 1) {
      ...
      accumulated_s += std::hypot(x1 - pre_point.x(), y1 - pre_point.y());///< 当前点与前一个点的距离
    }
    point->set_s(accumulated_s);
    point->set_theta(
        std::atan2(3 * path_c3 * x1 * x1 + 2 * path_c2 * x1 + path_c1, 1));///< y求导得该点处斜率，通过反正切求得角度
    point->set_kappa(GetKappa(path_c1, path_c2, path_c3, x1)); ///< 求该点的曲率

    const double k1 = GetKappa(path_c1, path_c2, path_c3, x1 - 0.0001);
    const double k2 = GetKappa(path_c1, path_c2, path_c3, x1 + 0.0001);
    point->set_dkappa((k2 - k1) / 0.0002);///< 求该点曲率的变化率
  }
  ...
  perceived_left_width_ =
      common::math::Clamp(perceived_left_width_, FLAGS_min_lane_half_width,///< 1.5
                          FLAGS_max_lane_half_width);///< 2.0  感知的左宽度为1.5-2.0m
  perceived_right_width_ = perceived_left_width_; ///< 感知的右宽度设置成和左宽度相等
}
...
bool NavigationLane::CreateMap(const MapGenerationParam &map_config,
  ...
  auto create_map_func = [&](const NaviPathTuple &navi_path_tuple) {
    ...
    lane->mutable_id()->set_id(std::to_string(navi_path->path_priority()) +
                               "_" + path.name());///< 0_Path from navigation line index 0
    ...
    if (FLAGS_relative_map_generate_left_boundray) {/// true
    ...
    double lane_left_width = 5;///修改   //const double lane_left_width = 5;//const double lane_left_width = std::get<1>(navi_path_tuple);
    double lane_right_width = 10;///修改 //const double lane_right_width = std::get<2>(navi_path_tuple);
    ///int n = 0;///添加
    
    ///添加--- 假设抛物线满足方程y=k*(x-a)*(x-a)+b ,(a,b)是抛物线的顶点
    int l = 30;/// 生成的曲线距离
    double b = 10;///b是曲线顶点距离车道的距离
    double a = l/2;///a是抛物线对应x轴的中点
    double k = (lane_left_width - b)/pow((l - a),2);///k是二次项系数
    ///---添加
    ...
    for (const auto &path_point : path.path_point()) {
      ...
      ///添加---
      lane_left_width = k * pow((path_point.s() - a),2) + b;  ///相当于方程y=k*(x-a)*(x-a)+b             
      if (path_point.s()>l){///超过设定的范围（30），宽度固定为5
        lane_left_width = 5;
      }
      ///---添加
...
void NavigationLane::UpdateStitchIndexInfo() {
  ...
  int navigation_line_num = navigation_info_.navigation_path_size();/// 参考线数量
  ...
  for (int i = 0; i < navigation_line_num; ++i) {
    ...
    if (!navigation_info_.navigation_path(i).has_path() ||
      ...
      continue;/// 参考线中至少也要有100个点
    }

    double min_distance = std::numeric_limits<double>::max(); /// 1.79769e+308
    StitchIndexPair min_index_pair = std::make_pair(-1, -1);

    int path_size = navigation_path.path_point_size(); /// 参考线信息中一共有多少个点
    const double start_s = navigation_path.path_point(0).s();/// 参考线信息中第一个点处的s
    const double end_s = navigation_path.path_point(path_size - 1).s();/// 参考线信息最后一个点处的s
    for (int m = 0; m < path_size; ++m) {
      double forward_s = navigation_path.path_point(m).s() - start_s; /// 某个点到起点的距离
      if (forward_s > FLAGS_max_len_from_navigation_line) {/// 250.0
        break;
      }

      for (int n = path_size - 1; n >= 0; --n) {
        double reverse_s = end_s - navigation_path.path_point(n).s();/// 某个点到终点的距离
        if (reverse_s > FLAGS_max_len_from_navigation_line) {/// 250.0
          break;
        }
        if (m == n) {
          break;
        ...
        double current_distance = DistanceXY(navigation_path.path_point(m),
                                             navigation_path.path_point(n));/// 求m,n两点的欧式距离
    ...
    if (min_distance < FLAGS_min_lane_half_width) {/// 1.5
      AINFO << "The stitching pair is: (" << min_index_pair.first << ", "
            << min_index_pair.second << ") for the navigation line: " << i;
      stitch_index_map_[i] = min_index_pair; ///参考线起点变成了m，终点变成了n
    }
  }
}
