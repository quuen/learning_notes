...
Status Frame::InitFrameData() {
  ...
  if (AdapterManager::GetPrediction() &&
    ...
    prediction.CopyFrom(AdapterManager::GetPrediction()->GetLatestObserved());        
    //auto prediction = AdapterManager::GetPrediction()->GetLatestObserved();
...
void Frame::RecordInputDebug(planning_internal::Debug *debug) {
  ...
  if (AdapterManager::GetPrediction() &&
      !AdapterManager::GetPrediction()->Empty()) {
    auto prediction = AdapterManager::GetPrediction()->GetLatestObserved();
    planning_debug_data->mutable_prediction_header()->CopyFrom(prediction.header());
  }
...
void Frame::ReadTrafficLights() {
  ...
  //auto traffic_light_detection = nullptr;
  if (AdapterManager::GetTrafficLightDetection() &&
        !AdapterManager::GetTrafficLightDetection()->Empty()) {
    auto traffic_light_detection = AdapterManager::GetTrafficLightDetection()->GetLatestObservedPtr();
...
void Frame::ReadPadMsgDrivingAction() {
  if (AdapterManager::GetPlanningPad() &&
        !AdapterManager::GetPlanningPad()->Empty()) {
    auto padMsg = AdapterManager::GetPlanningPad()->GetLatestObservedPtr();
    if (padMsg) {
      if (padMsg->has_action()) {
        pad_msg_driving_action_ = padMsg->action();
      }
    }
  }
}
