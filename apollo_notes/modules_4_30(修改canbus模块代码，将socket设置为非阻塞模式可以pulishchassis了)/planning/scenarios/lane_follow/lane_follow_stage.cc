...
Status LaneFollowStage::PlanOnReferenceLine(
  ...
  auto ret = Status::OK();
  AERROR<<task_list_.size()<<"   task_list.size";
  //if(task_list_[3]==nullptr) AERROR<<"task is NULL????????";
  //AERROR<<task_list_.back()->Name();
  for (auto* optimizer : task_list_) {
    const double start_timestamp = Clock::NowInSeconds();
    //if(optimizer==nullptr) AERROR<<"task is NULL!";
    AERROR << "task name:"<<optimizer->Name();
