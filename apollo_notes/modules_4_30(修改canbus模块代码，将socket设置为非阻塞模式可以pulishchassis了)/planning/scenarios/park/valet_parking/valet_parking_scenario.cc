...
bool ValetParkingScenario::IsTransferable(const Frame& frame,
  ...
  // modify by lx
  const double trigger_x = 561849;
  const double trigger_y = 4146883;
  double distance = 0;
  if (AdapterManager::GetLocalization() &&
    ...
    auto x = localization.pose().position().x();
    auto y = localization.pose().position().y();
    double delta_x = x - trigger_x;
    double delta_y = y - trigger_y;
    distance = std::sqrt(std::pow(delta_x,2) + std::pow(delta_y,2));
  }
  else{
    return false;
  }

  ///<  距离触发点太远，不触发park场景, 这个改成区域触发把
