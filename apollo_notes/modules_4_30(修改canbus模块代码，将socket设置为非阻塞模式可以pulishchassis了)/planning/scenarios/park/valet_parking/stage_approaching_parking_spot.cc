...
Stage::StageStatus StageApproachingParkingSpot::Process(
  ...
  AERROR << "stage: StageApproachingParkingSpot";
  CHECK_NOTNULL(frame);
  GetContext()->target_parking_spot_id.clear();
  if (AdapterManager::GetRoutingResponse() &&
      !AdapterManager::GetRoutingResponse()->Empty()) {
    auto routing_response = AdapterManager::GetRoutingResponse()->GetLatestObserved();
    if (routing_response.routing_request().has_parking_space() &&
        routing_response.routing_request().parking_space().has_id()) {
        GetContext()->target_parking_spot_id = "11543";//routing_response.routing_request().parking_space().id().id();
    } else {
