...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  CHECK(apollo::common::util::GetProtoFromFile(FLAGS_smoother_config_filename,
                                        &smoother_config_))
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
    std::list<ReferenceLine> *reference_lines,
    std::list<hdmap::RouteSegments> *segments) {
  const relative_map::MapMsg &relative_map = AdapterManager::GetRelativeMap()->GetLatestObserved();
  ...
  auto *adc_navigation_path = apollo::common::util::FindOrNull(
      relative_map.navigation_path(), adc_lane_id);
