...
Status OpenSpaceRoiDecider::Process(Frame *frame) {
  ...
  if (roi_type == OpenSpaceRoiDeciderConfig::PARKING) {
    ...
    cur_x = localization.pose().position().x();
    cur_y = localization.pose().position().y();
    cur_heading = localization.pose().heading();
    target_parking_spot_id_ = "11543";

    if (!GetParkingSpot(frame, &spot_vertices, &nearby_path)) {
      const std::string msg = "Fail to get parking spot from map";
      AERROR << msg;
      return Status(ErrorCode::PLANNING_ERROR, msg);
    }

    SetOrigin(frame, spot_vertices);///< 以左上角为基准点

    SetParkingSpotEndPose(frame, spot_vertices);///< 入库后停止时的位姿，和上面origin信息作为混合A*输入
    ...
    if (!GetParkingBoundary(frame, spot_vertices, nearby_path, &roi_boundary)) {
      ...
      const std::string msg = "localization is empty!";
      AERROR << msg;
      return Status(ErrorCode::PLANNING_ERROR, msg);
   }
  } else {
...
void OpenSpaceRoiDecider::SetOrigin(
  ...
  auto left_top = vertices[0];//(5,-5)
  auto right_top = vertices[3];//(11,-5)
  // rotate the points to have the lane to be horizontal to x axis positive
  // direction and scale them base on the origin point
  Vec2d heading_vec = right_top - left_top;//(6,0)
  frame->mutable_open_space_info()->set_origin_heading(heading_vec.Angle());//0
  frame->mutable_open_space_info()->mutable_origin_point()->set_x(left_top.x());
  frame->mutable_open_space_info()->mutable_origin_point()->set_y(left_top.y());
}
...
void OpenSpaceRoiDecider::SetParkingSpotEndPose(
    Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices) {
  auto left_top = vertices[0];//(5,-5)
  auto left_down = vertices[1];//(5,-15)
  auto right_down = vertices[2];//(11,-15)
  auto right_top = vertices[3];//(11,-5)

  const auto &origin_point = frame->open_space_info().origin_point();//(5,-5)
  const auto &origin_heading = frame->open_space_info().origin_heading();//0

  // End pose is set in normalized boundary 应该是都转到了以左上角为原点的，有ENU转到左上为原点的坐标系
  left_top -= origin_point;
  left_top.SelfRotate(-origin_heading);//(0,0)
  left_down -= origin_point;
  left_down.SelfRotate(-origin_heading);//(0,-10)
  right_top -= origin_point;
  right_top.SelfRotate(-origin_heading);//(6,0)
  right_down -= origin_point;
  right_down.SelfRotate(-origin_heading);//(6,-10)

  // TODO(Jinyun): adjust end pose setting for more parking spot configurations
  double parking_spot_heading = (left_down - left_top).Angle();//-pi/2
  double end_x = (left_top.x() + right_top.x()) / 2.0;//  3
  double end_y = 0.0;
  ...
  if (config_.open_space_roi_decider_config().parking_inwards()) {
    ...
    end_pose->push_back(
        common::math::NormalizeAngle(parking_spot_heading + M_PI));//-pi/2
  }
  // end pose velocity set to be zero
  end_pose->push_back(0.0);//(3,-7.3,-pi/2)
}
...
void OpenSpaceRoiDecider::GetRoadBoundary(
  ...
  const double dx = -cur_x;//-original_pose_.position().x();
  const double dy = -cur_y;//-original_pose_.position().y();
  auto enu_to_flu_func = [this, dx, dy](double enu_x, double enu_y,
                                      double enu_theta, double *flu_x,
                                      double *flu_y, double *flu_theta) {
  if (flu_x != nullptr && flu_y != nullptr) {
    common::math::RotateAxis(cur_heading, enu_x + dx, enu_y + dy,
                              flu_x, flu_y);
  }

  if (flu_theta != nullptr) {
    *flu_theta = common::math::NormalizeAngle(
        common::math::NormalizeAngle(enu_theta) - cur_heading);
  }
  }; /// 世界坐标系转换成车体坐标系的匿名函数  

  double flu_left_first_x = 0.0;
  double flu_left_first_y = 0.0;
  double flu_left_first_theta = 0.0;
  enu_to_flu_func(561849, 4146888, 0, &flu_left_first_x,
                  &flu_left_first_y, &flu_left_first_theta);
  Vec2d point1(flu_left_first_x,flu_left_first_y); 
  //Vec2d point1(0,5);
  left_lane_boundary->push_back(point1);
  
  double flu_left_second_x = 0.0;
  double flu_left_second_y = 0.0;
  double flu_left_second_theta = 0.0;
  enu_to_flu_func(561865, 4146888, 0, &flu_left_second_x,
                  &flu_left_second_y, &flu_left_second_theta);
  Vec2d point2(flu_left_second_x,flu_left_second_y);  
  //Vec2d point2(16,5);
  left_lane_boundary->push_back(point2);
  // (*left_lane_boundary)[0] = {0,5};
  // (*left_lane_boundary)[1] = {16,5};

  double flu_right_first_x = 0.0;
  double flu_right_first_y = 0.0;
  double flu_right_first_theta = 0.0;
  enu_to_flu_func(561849, 4146878, 0, &flu_right_first_x,
                  &flu_right_first_y, &flu_right_first_theta);
  Vec2d point3(flu_right_first_x,flu_right_first_y);  
  //Vec2d point3(0,-5);
  right_lane_boundary->push_back(point3);

  double flu_right_second_x = 0.0;
  double flu_right_second_y = 0.0;
  double flu_right_second_theta = 0.0;
  enu_to_flu_func(561865, 4146878, 0, &flu_right_second_x,
                  &flu_right_second_y, &flu_right_second_theta);
  Vec2d point4(flu_right_second_x,flu_right_second_y);  
  //Vec2d point4(16,-5);
  right_lane_boundary->push_back(point4);
  // (*right_lane_boundary)[0] = {0,-5};
  // (*right_lane_boundary)[1] = {16,-5};


  /// 左右边界进行坐标转换，都转换到车库左上角点
...
bool OpenSpaceRoiDecider::GetParkingBoundary(
 ...
 /// 这边需要参考线上的信息 如s、l，得出车库四个顶点投影在车道参考线上的s、l
  ...
  GetRoadBoundary(origin_point, origin_heading,&left_lane_boundary, &right_lane_boundary);
  ...
  // }; /// 世界坐标系转换成车体坐标系的匿名函数  
   ...
   /// boundary_points是这么存数据，|右车道边界|停车区域|左车道边界
  std::vector<Vec2d> boundary_points;
  AERROR << "parking space is on the right lane boundary in OpenSpaceROI";

  boundary_points.push_back(right_lane_boundary[0]);

  // double flu_right_mid_l_x = 0.0;
  // double flu_right_mid_l_y = 0.0;
  // double flu_right_mid_l_theta = 0.0;
  // enu_to_flu_func(561854, 4146878, 0, &flu_right_mid_l_x,
  //                 &flu_right_mid_l_y, &flu_right_mid_l_theta);
  //boundary_points.push_back({flu_right_mid_l_x,flu_right_mid_l_y});//{4,-5}
  boundary_points.push_back({-1,0});//{4,-5} 和下面这两个点都是基于车库左上角的坐标系了
  ...
  // double flu_right_mid_r_x = 0.0;
  // double flu_right_mid_r_y = 0.0;
  // double flu_right_mid_r_theta = 0.0;
  // enu_to_flu_func(561854, 4146878, 0, &flu_right_mid_r_x,
  //                 &flu_right_mid_r_y, &flu_right_mid_r_theta);
  // boundary_points.push_back({flu_right_mid_r_x,flu_right_mid_r_y});//{12,-5}
  boundary_points.push_back({7,0});//{12,-5}

  boundary_points.push_back(right_lane_boundary[1]);
  boundary_points.push_back(left_lane_boundary[1]);
  boundary_points.push_back(left_lane_boundary[0]);

  // reinsert the initial point to the back to from closed loop
  boundary_points.push_back(right_lane_boundary.front());

  AERROR << boundary_points.size() << " :is 11 points?";
  
  // disassemble line into line2d segments
  std::vector<Vec2d> segment1{boundary_points[0],boundary_points[1]};
  roi_parking_boundary->push_back(segment1);

  std::vector<Vec2d> left_stitching_segment{boundary_points[1], left_top};
  ...
  std::vector<Vec2d> right_stitching_segment{right_top, boundary_points[6]};
  roi_parking_boundary->push_back(right_stitching_segment);


  std::vector<Vec2d> segment2{boundary_points[6],boundary_points[7]};
  roi_parking_boundary->push_back(segment2);
 

  std::vector<Vec2d> segment3{boundary_points[8],boundary_points[9]};
  roi_parking_boundary->push_back(segment3);
  ...
  vehicle_xy.SelfRotate(-origin_heading);///< 这边应该是作了坐标系转换，转到了车库左上角为原点的坐标系
...
bool OpenSpaceRoiDecider::GetParkingSpot(Frame *const frame,
  ...
  //auto original_pose_ = VehicleStateProvider::instance()->original_pose();
...
//   }
  ...
  // open upward
  double flu_left_top_x = 0.0;
  double flu_left_top_y = 0.0;
  double flu_left_top_theta = 0.0;
  enu_to_flu_func(561854, 4146878, 0, &flu_left_top_x,
                  &flu_left_top_y, &flu_left_top_theta);
  //Vec2d left_top(5,-5);///< FLU坐标下的点
  Vec2d left_top(flu_left_top_x,flu_left_top_y);

  double flu_left_down_x = 0.0;
  double flu_left_down_y = 0.0;
  double flu_left_down_theta = 0.0;
  enu_to_flu_func(561854, 4146868, 0.5, &flu_left_down_x,
                  &flu_left_down_y, &flu_left_down_theta);  
  //Vec2d left_down(5,-15); 
  Vec2d left_down(flu_left_down_x,flu_left_down_y); 


  double flu_right_down_x = 0.0;
  double flu_right_down_y = 0.0;
  double flu_right_down_theta = 0.0;
  enu_to_flu_func(561860, 4146868, -2.64, &flu_right_down_x,
                  &flu_right_down_y, &flu_right_down_theta);
  //Vec2d right_down(11,-15); 
  Vec2d right_down(flu_right_down_x,flu_right_down_y);

  double flu_right_top_x = 0.0;
  double flu_right_top_y = 0.0;
  double flu_right_top_theta = 0.0;
  enu_to_flu_func(561860, 4146878, 0.5, &flu_right_top_x,
                  &flu_right_top_y, &flu_right_top_theta);
  //Vec2d right_top(11,-5); 
  Vec2d right_top(flu_right_top_x,flu_right_top_y); 



  std::array<Vec2d, 4> parking_vertices{left_top, left_down, right_down,
                                        right_top};
  //std::array<Vec2d, 4> parking_vertices{{5,-5}, {5,-15}, {11,-15},{11,-5}};
...
bool OpenSpaceRoiDecider::CheckDistanceToParkingSpot(
  ...
  if (std::abs((left_bottom_point_s + right_bottom_point_s) / 2 -
    ...
    return true;///< 判断入库中心所处s处与车辆当前位置是否小于设置的阈值来触发停车
  } else {
    return false;
  }
}
