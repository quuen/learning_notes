...
Status OpenSpaceTrajectoryProvider::Process() {
  ...
  if (FLAGS_enable_open_space_planner_thread && !thread_init_flag_) {
    task_future_ = ThreadPool::pool()->push(
       std::bind(&OpenSpaceTrajectoryProvider::GenerateTrajectoryThread, this));    
  ...
  //   AERROR << "stitching_trajectory size is: " << stitching_trajectory.size();
  ...
  if (FLAGS_enable_open_space_planner_thread) {
    ...
    const auto& end_pose = open_space_info.open_space_end_pose();//(3,-7.3,pi/2,0)
    const auto& rotate_angle = open_space_info.origin_heading();//0
    const auto& translate_origin = open_space_info.origin_point();//(5,-5)
    ...
    AERROR << XYbounds.size() <<"  ::: " <<XYbounds[0]<<":"<<XYbounds[1]<<":"<<XYbounds[2]<<":"<<XYbounds[3];
