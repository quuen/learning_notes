...
GriddedPathTimeGraph::GriddedPathTimeGraph(
  ...
  AERROR << "sparse_unit_s_:"<<sparse_unit_s_ << "   dense_dimension_s_:"<<dense_dimension_s_;
...
Status GriddedPathTimeGraph::Search(SpeedData* const speed_data) {
  ...
  ///< 在对自车当前位置（原点）判断后，速度规划主要分为以下几步：构建代价表InitCostTable()，
  ///< 更新代价表cost_table_中每个节点的代价CalculateTotalCost()，
  ///< 查找代价最小的规划结束点并逆向构建路径、求取速度RetrieveSpeedProfile()。
  if (!InitCostTable().ok()) {
...
Status GriddedPathTimeGraph::InitCostTable() {
  ...
  AERROR << "cost_table=dimension_t_:"<<dimension_t_<<"  "<<"dimension_s_:"<<dimension_s_;
...
Status GriddedPathTimeGraph::CalculateTotalCost() {
  ...
  for (size_t c = 0; c < cost_table_.size(); ++c) {///< 表示时间轴
    ...
    if (count > 0) {
      ...
      for (size_t r = next_lowest_row; r <= next_highest_row; ++r) {///< 表示对应时间点的s
...
Status GriddedPathTimeGraph::RetrieveSpeedProfile(SpeedData* const speed_data) {
  ...
  for (const StGraphPoint& cur_point : cost_table_.back()) {
    //AERROR << "cur_point.total_cost="<<cur_point.total_cost();
