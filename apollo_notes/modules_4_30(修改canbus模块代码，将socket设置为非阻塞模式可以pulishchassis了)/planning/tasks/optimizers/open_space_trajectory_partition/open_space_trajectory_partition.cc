...
Status OpenSpaceTrajectoryPartition::Process() {
  ...
  AERROR << "stitching_trajectory size in partition is: " << stitched_trajectory_result.size();
  ...
  PartitionTrajectory(*interpolated_trajectory_result_ptr,
                      partitioned_trajectories);///< 到这都是没问题啊，轨迹全部partition了
  //AERROR<<(*partitioned_trajectories)[0].size() << '    '<<(*partitioned_trajectories)[1].size()<<endl;
  ...
  if (!open_space_status.position_init() &&
    ...
    AdjustRelativeTimeAndS(open_space_info.partitioned_trajectories(), 0, 0,
                           mutable_trajectory, chosen_partitioned_trajectory);///<修改参数0可以选择发布哪段轨迹
    return Status::OK();
  }
...
bool OpenSpaceTrajectoryPartition::EncodeTrajectory(
  ...
  *encoding = apollo::common::util::StrCat(
      // init point
      init_point_x, "_", init_point_y, "_", init_point_heading, "/") +
      // last point
      apollo::common::util::StrCat(last_point_x, "_", last_point_y, "_", last_point_heading);
  return true;
}
...
void OpenSpaceTrajectoryPartition::PartitionTrajectory(
  ...
  *gear =
  ...
  for (size_t i = 0; i < horizon - 1; ++i) {
    ...
    if (cur_gear != *gear) {//< 档位变换的节点处就是partition的标准，分为另一部分轨迹
      LoadTrajectoryPoint(trajectory_point, *gear, &last_pos_vec, &distance_s,
                          trajectory);///< 档位变换前的最后一个节点先按照之前标准处理

      partitioned_trajectories->emplace_back();//又开了一块内存来存放gear change部分的轨迹
