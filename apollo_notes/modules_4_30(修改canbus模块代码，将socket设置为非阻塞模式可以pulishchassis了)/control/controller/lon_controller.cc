    ...
    acceleration_cmd =
        (chassis->gear_location() == canbus::Chassis::GEAR_REVERSE)
            ? std::max(acceleration_cmd,
                       -lon_controller_conf.standstill_acceleration())///< 静止加速度
...
void LonController::ComputeLongitudinalErrors(
  ...
  ///< matched_point表示当前车辆位置投影在在规划轨迹上的那个匹配点
  ///< reference_point表示匹配的那个点，后面运算的参考点
  ///< preview_point表示预瞄的点
  ...
  ///< 将车辆当前位置转换为sl坐标。///以轨迹上的匹配点为原点的坐标系///
