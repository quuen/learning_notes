...
Status Control::Init() {
  ...
  CHECK(common::util::GetProtoFromFile(FLAGS_control_conf_file, &control_conf_))
      << "Unable to load control conf file: " + FLAGS_control_conf_file;///< "modules/control/conf/lincoln.pb.txt"

  AINFO << "Conf file: " << FLAGS_control_conf_file << " is loaded.";

  AdapterManager::Init(FLAGS_control_adapter_config_filename);///< "modules/control/conf/adapter.conf",创建要发布和订阅的话题
...
Status Control::Start() {
  ...
  timer_ = AdapterManager::CreateTimer(
      ros::Duration(control_conf_.control_period()), &Control::OnTimer, this);///< 以100hz的频率进行控制
...
Status Control::ProduceControlCommand(ControlCommand *control_command) {
  Status status = CheckInput();///< 导航模式坐标在VehicleStateProvider类里作了处理
  // check data
...
//////////< 倒车速度也变成0了啊！！！
  ...
  VehicleStateProvider::instance()->Update(localization_, chassis_);//导航模式下，只把底盘信息给更新，localization只更新了时间戳其他信息使用默认值0
