...
double Interpolation2D::Interpolate(const KeyType &xy) const {
  double max_x = xyz_.rbegin()->first; ///<  找出标定表中最大的速度值
  double min_x = xyz_.begin()->first;   
  if (xy.first >= max_x - kDoubleEpsilon) {///< 如果速度超出了标定表的最大速度范围，直接取边界处的油门值
    return InterpolateYz(xyz_.rbegin()->second, xy.second);
  }
  if (xy.first <= min_x + kDoubleEpsilon) {///< 如果速度低于标定表的下边界，直接区边界处的油门值
...
double Interpolation2D::InterpolateYz(const std::map<double, double> &yz_table,
  ...
  return InterpolateValue(z_before, y_diff_before, z_after, y_diff_after);///< 一维线性插值
}
