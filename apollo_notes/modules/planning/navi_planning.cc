...
Status NaviPlanning::Init() {
  CHECK(apollo::common::util::GetProtoFromFile(FLAGS_planning_config_file,///< planning_config.pb.txt
                                               &config_))
  ...
  planner_dispatcher_->Init();///< 这里可以更改planner
...
Status NaviPlanning::InitFrame(const uint32_t sequence_num,
  ...
  frame_.reset(new Frame(sequence_num, planning_start_point, start_time,
                         vehicle_state, reference_line_provider_.get()));///< frame_对象包含一帧planning所需的所有信息
  auto status = frame_->Init();  ///< 初始化vehicle_state_、point(ENU)               
  if (!status.ok()) {
...
Status NaviPlanning::Start() {
  timer_ =
      AdapterManager::CreateTimer(ros::Duration(1.0 / FLAGS_planning_loop_rate),///< 10Hz
...
void NaviPlanning::RunOnce() {
  ...
  hdmap_ = HDMapUtil::BaseMapPtr();///< 导航模式下获取相对地图指针
  ...
  Status status =
      VehicleStateProvider::instance()->Update(localization, chassis);///< 更新车辆状态，注意导航模式下更新的数据

  auto vehicle_config = ComputeVehicleConfigFromLocalization(localization);///< 获取车辆ENU下的(x,y)、heading

  if (last_vehicle_config_.is_valid_ && vehicle_config.is_valid_) {///< 此处为什么不直接转换成当前点车辆坐标系下，为什么要先转换成上一个点的车辆坐标系，在转换成当前点车辆坐标系
    auto x_diff_map = vehicle_config.x_ - last_vehicle_config_.x_;///地图坐标系(ENU)下的横纵向误差
    ...
    ///< 当前点相对于上一个点的平移矩阵
    auto x_diff_veh = cos_map_veh * x_diff_map + sin_map_veh * y_diff_map;///< 两点在上一个车辆点坐标系下的纵向误差
    auto y_diff_veh = -sin_map_veh * x_diff_map + cos_map_veh * y_diff_map;///< 两点在上一个车辆点坐标系下的横向误差

    auto theta_diff = vehicle_config.theta_ - last_vehicle_config_.theta_;///< 从前一个点到当前点的旋转角，进而求旋转矩阵

    TrajectoryStitcher::TransformLastPublishedTrajectory(
        x_diff_veh, y_diff_veh, theta_diff, last_publishable_trajectory_.get());///< 将上一次规划的轨迹点转换到当前位置的车辆坐标系下
  }
  last_vehicle_config_ = vehicle_config;

  VehicleState vehicle_state =
      VehicleStateProvider::instance()->vehicle_state();///< 车辆当前状态
  ...
  EgoInfo::instance()->Update(stitching_trajectory.back(), vehicle_state,
                              frame_->obstacles());///< 更新起始点为stitching_trajectory.back()，车辆状态
...
NaviPlanning::VehicleConfig NaviPlanning::ComputeVehicleConfigFromLocalization(
  ...
  if (localization.pose().has_heading()) {///< 有heading就用，没有就用四元数计算航向
