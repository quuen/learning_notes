...
void TrajectoryStitcher::TransformLastPublishedTrajectory(
  ...
  // R^-1
  double cos_theta = std::cos(theta_diff);   // auto theta_diff = vehicle_config.theta_ - last_vehicle_config_.theta_;
  double sin_theta = -std::sin(theta_diff);  // 角度差取负号，即以当前点为基准的车辆坐标系
  ...
  std::for_each(prev_trajectory->trajectory_points().begin(),
                ...
                [&cos_theta, &sin_theta, &tx, &ty,
                 &theta_diff](common::TrajectoryPoint& p) {///< p就是上一帧的规划轨迹点
                  auto x = p.path_point().x();
                  auto y = p.path_point().y();
                  auto theta = p.path_point().theta();///< 轨迹点在上一帧车辆坐标系的角度

                  auto x_new = cos_theta * x - sin_theta * y + tx;///< 根据p_new=R*p_old + t;将上一次的轨迹点转换成当前车辆坐标系下的点
...
std::vector<TrajectoryPoint> TrajectoryStitcher::ComputeStitchingTrajectory(
  ...
  if (prev_trajectory_size == 0) {///< 如果上帧轨迹点的数量为0，需要重新规划
  ...
  if (time_matched_index == 0 &&///< 当前时间和上帧轨迹头时间戳的相对时间超过了上帧轨迹的最后一个点的相对时间，说明需要重新规划了
...
std::pair<double, double> TrajectoryStitcher::ComputePositionProjection(
  ...
  frenet_sd.first = v.InnerProd(n) + p.path_point().s();///< 内积
  frenet_sd.second = v.CrossProd(n);///< 外积
  return frenet_sd;
}
