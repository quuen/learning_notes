...
bool WaypointSampler::SamplePathWaypoints(
  ...
  const float kMinSampleDistance = 40.0;///< 最小采样距离
  const float total_length = std::fmin(///< 采样到累计距离s坐标为total_length处，大概是规划起点顺延40m左右
      init_sl_point_.s() + std::fmax(init_point.v() * 8.0, kMinSampleDistance),
      reference_line_info_->reference_line().Length());///< 总长度 = min(初始点s + max(初始速度 × 8, 最小采样距离), 参考线长度）
  const auto &vehicle_config =
      common::VehicleConfigHelper::instance()->GetConfig();
  const float half_adc_width = vehicle_config.vehicle_param().width() / 2.0;///< 半车宽
  const size_t num_sample_per_level =///< 每层采样的点数
  ...
  constexpr float kSamplePointLookForwardTime = 4.0;///< 采样前视时长
 ...
 ///< step_length_min(默认值：8) <= 采样步长 <= step_length_max(默认值：15)
  ...
  for (std::size_t i = 0; accumulated_s < total_length; ++i) {
    ...
    } else {// 其他情形，从右采样边界到左采样边界，按照每层level采样点数进行均匀采样，并将结果存储到横向采样距离数组
